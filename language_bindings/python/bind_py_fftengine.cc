   ...
   public:
    ...
    std::unique_ptr<FFTEngineBase> clone() const final {
        throw muFFT::FFTEngineError(
            "Python version of FFTEngine cannot be cloned");
    }
};
...
template <class Engine, bool with_ghosts>
class AddEngineConstructor {
   public:
    static void add(py::class_<Engine, std::shared_ptr<Engine>, FFTEngineBase> &
                        fft_engine) {
        fft_engine.def(
            py::init([](std::vector<Index_t> nb_grid_pts,
...
class AddEngineConstructor<Engine, true> {
   public:
    static void add(py::class_<Engine, std::shared_ptr<Engine>, FFTEngineBase> &
                        fft_engine) {
        fft_engine.def(
...
template <class Engine, bool with_ghosts = false>
void add_engine_helper(py::module & mod, const std::string & name) {
    ...
    AddEngineConstructor<Engine, with_ghosts>::add(fft_engine);
    fft_engine.def("fft", &Engine::fft)
        ...
        .def("register_real_space_field",
             (FFTEngineBase::RealField_t & (Engine::*)(const std::string &,
                                                       const Shape_t &,
                                                       const std::string &)) &
                 Engine::register_real_space_field,
               ...
               py::array_t<Complex> & output_array) {
                ...
                if (info.shape.size() < static_cast<size_t>(dim)) {
                    ...
                    s << "Input array has " << info.shape.size()
                      << " dimensions "
                      << "but FFT engine was set up for " << dim
                      << " dimensions.";
        ...
        .def(
            ...
            "real_input_array"_a, "real_output_array"_a,
            "Perform inverse FFT of the input array into the output "
            "array.");
}
