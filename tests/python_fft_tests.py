...
class FFTCheck(unittest.TestCase):
        ...
        for engine_str in self.engines:
            if muFFT.has_mpi:
                ...
                try:
                    nb_dof = 6
                    engine = muFFT.FFT(
                        [6 * s, 4 * s], engine=engine_str, communicator=MPI.COMM_WORLD
                    )
            ...
            try:
                engine = muFFT.FFT(
                    [6 * s, 4 * s], engine=engine_str, communicator=self.communicator
                )
                engine.create_plan(nb_dof)
            except muFFT.UnknownFFTEngineError:
                # This FFT engine has not been compiled into the code. Skip
            ...
            for nb_grid_pts, dims in self.grids:
                for s in [1, self.communicator.size]:
                    ...
                    if len(nb_grid_pts) == 2:
                        ...
                        raise RuntimeError(
                            "Cannot handle {}-dim transforms".format(len(nb_grid_pts))
                        )
                    ...
                    out_msp = np.empty(
                        [*dims, *engine.nb_fourier_grid_pts], dtype=complex, order="f"
                    )
        ...
        for engine_str in self.engines:
            for nb_grid_pts, dims in self.grids:
                for s in [1, self.communicator.size]:
                    ...
                    # Separately test convenience interface
                    out_msp = np.empty(
                        [*dims, *engine.nb_subdomain_grid_pts], dtype=float
                    )
        ...
        for engine_str in self.engines:
            for nb_grid_pts, dims in self.grids:
                for s in [1, self.communicator.size]:
                    ...
                    fc.initialise(
                        tuple(engine.nb_domain_grid_pts),
                    ...
                    self.assertLess(
                        err,
                        tol,
                        msg="{} engine, nb_grid_pts = {}, s = {}".format(
                            engine_str, nb_grid_pts, s
                        ),
                    )
                    np.testing.assert_allclose(
                        indata, in_field.s
                    )  # Check that input is not destroyed
        ...
        for engine_str in self.engines:
            for nb_grid_pts, dims in self.grids:
                for s in [1, self.communicator.size]:
                    ...
                    fourier_field.array(muGrid.Pixel)[...] = global_in_arr[
                        (..., *engine.fourier_slices)
                    ]
                    ...
                    err = np.linalg.norm(
                        out_ref - out_field.array(muGrid.Pixel) * engine.normalisation
                    )
                    self.assertLess(err, tol, msg="{} engine".format(engine_str))
                    np.testing.assert_allclose(
                        fourierdata, fourier_field.s
                    )  # Check that input is not destroyed
    ...
    def test_nb_components1_forward_transform(self):
        for engine_str in self.engines:
            for nb_grid_pts, dims in self.grids:
                ...
                try:
                    engine = muFFT.FFT(
                        nb_grid_pts, engine=engine_str, communicator=self.communicator
                    )
                    engine.create_plan(np.prod(dims))
                except muFFT.UnknownFFTEngineError:
                    # This FFT engine has not been compiled into the code. Skip
                ...
                # Separately test convenience interface
                out_msp = engine.register_fourier_space_field("out_msp", dims)
                self.assertFalse(out_msp.array().flags.owndata)
                engine.fft(in_arr, out_msp)

                # Check that the output array does not have a unit first dimension
                assert (
                    np.squeeze(out_msp).shape == engine.nb_fourier_grid_pts
                ), "{} not equal to {}".format(
                    out_msp.shape, engine.nb_fourier_grid_pts
                )
    ...
    def test_nb_components1_forward_transform(self):
        for engine_str in self.engines:
            for nb_grid_pts, dims in self.grids:
                ...
                # Check that the output array does not have a unit first dimension
                self.assertEqual(
                    tuple(out_msp.shape), (1, 1) + engine.nb_fourier_grid_pts
                )
                ...
                # segmentation fault)
                self.assertRaises(RuntimeError, lambda: engine.fft(in_arr.ravel()))
    ...
    def test_nb_components1_reverse_transform(self):
        ...
        for engine_str in self.engines:
            for nb_grid_pts, dims in self.grids:
                ...
                in_arr = global_in_arr[engine.fourier_slices]
                in_arr.shape = (*dims, *engine.nb_fourier_grid_pts)
                out_msp = np.empty((1, *engine.nb_subdomain_grid_pts), order="f")
                engine.ifft(in_arr, out_msp)
                assert out_msp.shape == (
                    1,
                    *engine.nb_subdomain_grid_pts,
                ), "{} not equal to {}".format(
                    out_msp.shape, engine.nb_subdomain_grid_pts
                )

                # Convenience interface with flattened array (should not give a
                # segmentation fault)
                self.assertRaises(RuntimeError, lambda: engine.ifft(in_arr.ravel()))

    @unittest.skipIf(
        communicator.size > 1, "MPI parallel FFTs do not support 1D transforms"
    )
        ...
        for engine_str in self.engines:
            for nb_grid_pts, dims in self.grids:
                ...
                for s in [1, self.communicator.size]:
                    ...
                    freq = np.array(
                        np.meshgrid(
                            *(np.fft.fftfreq(n) for n in nb_grid_pts), indexing="ij"
                        )
                    )
    ...
    def test_2d_fftfreq(self):
        ...
        np.testing.assert_allclose(
            rarr[:, 0], np.cos(np.arange(nx) * 2 * np.pi / nx), atol=1e-12
        )
        ...
        np.testing.assert_allclose(
            rarr[0, :], np.cos(np.arange(ny) * 2 * np.pi / ny), atol=1e-12
        )
    ...
    def test_3d_fftfreq(self):
        ...
        qarr[
            np.logical_and(
                np.logical_and(
                    np.abs(np.abs(qx) * nx - 1) < 1e-6,
                    np.abs(np.abs(qy) * ny - 0) < 1e-6,
                ),
                np.abs(np.abs(qz) * nz - 0) < 1e-6,
            )
        ] = 0.5
        ...
        engine.ifft(qarr, rarr)
        np.testing.assert_allclose(
            rarr - rarr[:, 0, 0].reshape(-1, 1, 1), 0, atol=1e-12
        )
        np.testing.assert_allclose(rarr, np.cos(2 * np.pi * x), atol=1e-12)
        np.testing.assert_allclose(
            rarr[:, 0, 0], np.cos(np.arange(nx) * 2 * np.pi / nx), atol=1e-12
        )
        ...
        np.testing.assert_allclose(rarr, np.cos(2 * np.pi * y), atol=1e-12)
        np.testing.assert_allclose(
            rarr[0, :, 0], np.cos(np.arange(ny) * 2 * np.pi / ny), atol=1e-12
        )
        ...
        np.testing.assert_allclose(
            rarr[0, 0, :], np.cos(np.arange(nz) * 2 * np.pi / nz), atol=1e-12
        )
    ...
    def test_strides(self):
        for engine_str in self.engines:
            try:
                engine = muFFT.FFT(
                    [3, 5, 7], engine=engine_str, communicator=self.communicator
                )
                engine.create_plan(1)
            except muFFT.UnknownFFTEngineError:
                # This FFT engine has not been compiled into the code. Skip
            ...
            if engine_str == "fftwmpi":
                assert engine.subdomain_strides == (1, 4, 20), "{} - {}".format(
                    engine_str, engine.subdomain_strides
                )  # padding in first dimension
                assert engine.fourier_strides == (1, 14, 2), "{} - {}".format(
                    engine_str, engine.fourier_strides
                )  # transposed output
            elif engine_str == "pfft":
                assert engine.subdomain_strides == (1, 4, 20), "{} - {}".format(
                    engine_str, engine.subdomain_strides
                )  # padding in first dimension
                assert engine.fourier_strides == (7, 14, 1), "{} - {}".format(
                    engine_str, engine.fourier_strides
                )  # transposed output
            else:
                assert engine.subdomain_strides == (1, 3, 15), "{} - {}".format(
                    engine_str, engine.subdomain_strides
                )  # column-major
                assert engine.fourier_strides == (1, 2, 10), "{} - {}".format(
                    engine_str, engine.fourier_strides
                )  # column-major
    ...
    def test_raises_incompatible_buffer(self):
        ...
        for fft in ["fftw"]:
            try:
                engine = muFFT.FFT([3, 2], engine=fft, allow_temporary_buffer=False)
            except muFFT.UnknownFFTEngineError:
                # This FFT engine has not been compiled into the code. Skip
                # test.
                continue
    ...
    @unittest.skipIf(not muFFT.has_mpi, "Test requires FFTWMPI")
    def test_zero_grid_pts(self):
        ...
        try:
            engine = muFFT.FFT(
                nb_grid_pts, engine="fftwmpi", communicator=self.communicator
            )
            engine.create_plan(1)
        except muFFT.UnknownFFTEngineError:
            # This FFT engine has not been compiled into the code. Skip
        ...
        # Separately test convenience interface
        out_msp = np.empty(engine.nb_fourier_grid_pts, dtype=complex, order="f")
        engine.fft(in_arr, out_msp)
        err = np.linalg.norm(out_ref - out_msp)
...
class FFTCheckSerialOnly(unittest.TestCase):
    ...
    def test_rffth2c_2d_convenience_interface(self):
        ...
        result_real = np.empty(nb_grid_pts, dtype=float, order="f")
    ...
    def test_rffth2c_multiple_dofs(self):
        for nb_grid_pts, dims in self.grids:
            ...
            try:
                engine = muFFT.FFT(
                    nb_grid_pts, engine="fftw", communicator=self.communicator
                )
                engine.create_plan(np.prod(dims))
            except muFFT.UnknownFFTEngineError:
                # This FFT engine has not been compiled into the code. Skip
    ...
    def test_r2hc_incompatible_engines_raise(self):
        for engine in ["pocketfft"]:
            try:
                ...
                muFFT.UnknownFFTEngineError
            ):  # One of the engines is not installed, skip it
                continue
            # Allocate buffers and create plan for one degree of freedom
