...
namespace muFFT {
    ...
    void PocketFFTEngine::create_plan(const Index_t & nb_dof_per_pixel) {
        ...
        if (this->comm.size() > 1) {
            ...
            error << "PocketFFT engine does not support MPI parallel "
                     "execution, but "
                  << "a communicator of size " << this->comm.size()
                  << " was passed "
                  << "during construction";
    ...
    Index_t _get_offset(Index_t index, Shape_t shape, Shape_t strides) {
        if (index == 0) {
            // This catches corners-cases where shape and stride have different
            // sizes. This happens where the exactly one DOF, in which case
            // `get_strides` strips the component/sub-point dimensions from the
            // stride array.
            return 0;
        }
    ...
    void PocketFFTEngine::compute_fft(const RealField_t & input_field,
        ...
        if (nb_dof != output_field.get_nb_dof_per_pixel()) {
            std::stringstream error;
            error << "Input field has " << nb_dof
                  << " DOFs while output field has "
        ...
        // nx/2 + 1 grid points in Fourier space. This means we have to carry
        // out the real to complex transform on the (first) x-axis.
        ...
        for (size_t dim{0}; dim < nb_dim; ++dim) {
            ...
            pixel_output_strides[dim] =
                sub_pt_output_strides[nb_output_dof_dim + dim] *
                sizeof(Complex);
        }
        ...
        if (nb_dim == 1) {
            ...
            for (Index_t i{0}; i < nb_dof; ++i) {
                pocketfft::r2c(
                    real_shape, pixel_input_strides, pixel_output_strides, 0,
            ...
            // For n-dimensional transforms we need to carry out n-transforms in
            // the respective directions
            FourierField_t & tmp_field{
                this->fourier_space_field("pocketfft_tmp")};
            pocketfft::stride_t tmp_strides{tmp_field.get_strides(
                muGrid::IterUnit::Pixel, sizeof(Complex))};
            ...
            for (Index_t i{0}; i < nb_dof; ++i) {
                pocketfft::r2c(real_shape, pixel_input_strides, tmp_strides,
                               ...
                               1.0);  // additional multiplicative factor
                pocketfft::c2c(
                    fourier_shape, tmp_strides, pixel_output_strides, axes,
                    pocketfft::FORWARD,  // forward transform
                    tmp_field.data(),
                    output_field.data() + _get_offset(i, sub_pt_output_shape,
                                                      sub_pt_output_strides),
    ...
    void PocketFFTEngine::compute_ifft(const FourierField_t & input_field,
        ...
        if (nb_dim == 1) {
            ...
            for (Index_t i{0}; i < nb_dof; ++i) {
                pocketfft::c2c(
                    fourier_shape, pixel_input_strides, tmp_strides, axes,
                    pocketfft::BACKWARD,  // backward transform
                    input_field.data() + _get_offset(i, sub_pt_input_shape,
                                                     sub_pt_input_strides),
                    tmp_field.data(),
