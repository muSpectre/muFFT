...
namespace muFFT {
    ...
    void FFTWEngine::create_plan(const Index_t & nb_dof_per_pixel) {
        ...
        if (this->comm.size() > 1) {
            ...
            error
                << "FFTW engine does not support MPI parallel execution, but a "
                << "communicator of size " << this->comm.size()
                << " was passed "
                << "during construction";
    ...
    FFTWEngine::~FFTWEngine() noexcept {
        ...
        if (this->nb_engines == 0) {
            // TODO(Pastewka): Check with Lars why this crashes the
            // sequential(!) MPI tests
#ifndef WITH_MPI
            fftw_cleanup();
#endif
        }
    ...
    bool FFTWEngine::check_fourier_space_field(const FourierField_t & field,
        ...
        if (direction == FFTDirection::reverse && !this->allow_destroy_input) {
            // Serial FFTW does not support preserving input for
            // multi-dimensional transforms; we need to make a copy of the
            // buffer
            return false;
        } else {
    ...
    void FFTWEngine::compute_hcfft(const RealField_t & input_field,
                                   RealField_t & output_field) {
        fftw_execute_r2r(
            this->hcfft_plans.at(input_field.get_nb_dof_per_pixel()),
    ...
    void FFTWEngine::compute_ihcfft(const RealField_t & input_field,
                                    RealField_t & output_field) {
        fftw_execute_r2r(
            this->ihcfft_plans.at(input_field.get_nb_dof_per_pixel()),
            input_field.data(), output_field.data());
    }
