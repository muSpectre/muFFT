...
namespace muFFT {
    ...
    FFTWMPIEngine::FFTWMPIEngine(const IntCoord_t & nb_grid_pts,
        ...
        // res0, loc0 describe the decomposition of the first dimension of the
        // input data. Since FFTW expects a row-major grid but muFFT uses
        // column-major grids, this is actually the last dimension.
        this->nb_subdomain_grid_pts[dim - 1] = res0;
        this->subdomain_locations[dim - 1] = loc0;
        // Set the strides for the real space domain. The real space data needs
        // to contain an additional padding region if the size if odd numbered.
        // See
        ...
        if (dim > 1) {
            ...
            for (Index_t i = 2; i < dim; ++i) {
                this->subdomain_strides[i] = this->subdomain_strides[i - 1] *
                                             this->nb_subdomain_grid_pts[i - 1];
            }
        }
        ...
        // Set the strides for the Fourier domain. Since we are omitting the
        // last transpose, these strides reflect that the grid in the Fourier
        // domain is transposed. This transpose operation is transparently
        // handled by the Pixels class, which iterates consecutively of grid
        // locations but returns the proper (transposed or untransposed)
        // coordinates.
        if (dim > 1) {
            // cumulative strides over first dimensions
            Index_t s = this->fourier_strides[dim - 2];
            this->fourier_strides[dim - 1] = s;
            this->fourier_strides[dim - 2] =
                this->nb_fourier_grid_pts[dim - 1] * s;
        }
    ...
    void FFTWMPIEngine::create_plan(const Index_t & nb_dof_per_pixel) {
        ...
        if (this->fft_plans[nb_dof_per_pixel] == nullptr) {
            if (dim == 1) {
                throw FFTEngineError(
                    "r2c plan failed; MPI parallel FFTW does not "
                    "support 1D r2c FFTs");
            } else {
        ...
        if (this->ifft_plans[nb_dof_per_pixel] == nullptr) {
            if (dim == 1)
                throw FFTEngineError(
                    "c2r plan failed; MPI parallel FFTW does not "
                    "support 1D c2r FFTs");
            else
    ...
    FFTWMPIEngine::~FFTWMPIEngine() noexcept {
        ...
        // TODO(junge): We cannot run fftw_mpi_cleanup since also calls
        // fftw_cleanup and any running FFTWEngine will fail afterwards.
        // this->nb_engines--;
    ...
    FFTWMPIEngine::register_real_space_field(const std::string & unique_name,
         ...
         * enough. MPI parallel FFTW may request a workspace size larger than
         * the nominal size of the complex buffer.
         */
    ...
    /* ---------------------------------------------------------------------- */
    auto FFTWMPIEngine::register_real_space_field(
    ...
    muGrid::ComplexField & FFTWMPIEngine::register_fourier_space_field(
        ...
        auto & field{Parent::register_fourier_space_field(unique_name,
                                                          nb_dof_per_pixel)};
        /*
         * We need to check whether the fourier field provided is large
         * enough. MPI parallel FFTW may request a workspace size larger than
